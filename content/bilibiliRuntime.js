...
export class BilibiliRuntimeError extends Error {
...
function createDefaultMessageSender(runtime) {
  if (!runtime || typeof runtime.sendMessage !== "function") {
    throw new TypeError("Bilibili Runtime requires chrome.runtime.sendMessage().");
  }
  ...
  return (message) =>
    new Promise((resolve, reject) => {
      runtime.sendMessage(message, (response) => {
        ...
        if (runtimeError) {
          reject(new BilibiliRuntimeError(runtimeError.message));
          return;
        }
        resolve(response);
      });
...
/**
 * Compose the approved Bilibili Adapter, page-memory bindings and existing
 * collectors. Scoring and persistence remain entirely in the background.
 *
 * @param {{
 *   document?: Document,
...
export function createBilibiliRuntime(options = {}) {
  ...
  if (
    ...
    throw new TypeError("Bilibili Runtime dependencies are invalid.");
  }
  ...
  function readWallNow() {
    ...
    if (!Number.isFinite(timestamp) || timestamp < 0) {
      throw new TypeError("Bilibili wall clock must be finite and non-negative.");
    }
    return timestamp;
  }
  ...
  function readCollectorNow() {
    ...
    if (!Number.isFinite(timestamp)) {
      throw new TypeError("Bilibili collector clock must be finite.");
    }
    return timestamp;
  }
  ...
  function createCollectorGroup(session) {
    ...
    const click = createCandidateClickCollector({
      ...
      sendMessage(message) {
        ...
        reportBackgroundWrite(chosenWrite, "Bilibili 选择状态");
        if (state !== undefined) {
          reportBackgroundWrite(
            enqueueSignals(session, state),
            "Bilibili 选择信号"
  ...
  function pauseCollection(session) {
    ...
    lifecycle = "paused";
    emitStatus("paused", "Bilibili 采集已暂停；页面数据不会继续写入。");
  }
  ...
  function finalizeSession(session, reason) {
    ...
    session.finalizingPromise = (async () => {
      ...
      emitStatus(
        "finalized",
        `Bilibili Session 已因 ${reason} 完成结算。`,
        response.data
      );
      ...
      .catch((error) => {
        if (error?.cause?.code !== RESPONSE_ERROR_CODES.COLLECTION_PAUSED) {
          lifecycle = "degraded";
          emitStatus("degraded", `Bilibili Session 结算失败：${error.message}`);
        }
        throw error;
      })
      .finally(() => {
  ...
  async function discoverCandidates(candidates, context) {
    ...
    if (sessionIds.size !== 1) {
      throw new BilibiliRuntimeError("Bilibili Candidates 不属于同一 Session。");
    }
    ...
    emitStatus(
      "collecting",
      `Bilibili Runtime 正在采集 ${response.data.totalCandidateCount} 个候选项。`,
      response.data
    );
    return response.data;
  }
  ...
  async function reconcileCurrentPage(reason) {
    ...
    if (url === null || !adapter.canHandle(url, runtimeDocument)) {
      ...
      lifecycle = "inactive";
      emitStatus("inactive", "当前页面不是受支持的 Bilibili 搜索情境。");
      return null;
    }
    ...
    try {
      ...
      lifecycle = "degraded";
      emitStatus("degraded", `Bilibili 上下文读取失败：${error.message}`);
      return null;
    }
    ...
    try {
      ...
      lifecycle = "degraded";
      emitStatus("degraded", `Bilibili DOM 提取失败：${error.message}`);
      return null;
    }
    if (candidates.length === 0) {
      ...
      emitStatus(
        "waiting",
        `Bilibili 暂无可采集候选项（${reason}），等待页面动态结果。`
      );
      return null;
    }
  ...
  function scheduleReconcile(reason) {
    ...
    transitionTail = operation.catch((error) => {
      if (error?.cause?.code !== RESPONSE_ERROR_CODES.COLLECTION_PAUSED) {
        lifecycle = "degraded";
        emitStatus("degraded", `Bilibili Runtime 降级：${error.message}`);
      }
      return null;
    });
    return operation;
  ...
  function scheduleActiveFinalize(reason) {
    ...
    transitionTail = operation.catch((error) => {
      if (error?.cause?.code !== RESPONSE_ERROR_CODES.COLLECTION_PAUSED) {
        lifecycle = "degraded";
        emitStatus("degraded", `Bilibili Session 结算降级：${error.message}`);
      }
      return null;
    });
    return operation;
  ...
  async function start() {
    if (lifecycle !== "idle") {
      throw new BilibiliRuntimeError("Bilibili Runtime 已经启动或结束。");
    }
    lifecycle = "starting";
    ...
    try {
      ...
      lifecycle = "degraded";
      emitStatus("degraded", `Bilibili Adapter 无法观察页面：${error.message}`);
      return null;
    }
    ...
    try {
      ...
      lifecycle = "degraded";
      emitStatus("degraded", `Bilibili Runtime 启动降级：${error.message}`);
      return null;
    }
  }
  ...
  function cleanup() {
    ...
    activeSession = null;
    lifecycle = "cleaned";
    emitStatus("cleaned", "Bilibili Runtime 已清理。");
  }
