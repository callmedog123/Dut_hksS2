...
打开 Side Panel，在“采集控制”选择“暂停采集”或“恢复采集”。暂停状态会保存到 Settings；后台拒绝新的 Candidate、信号、点击、普通结算以及 Worker 恢复结算写入。暂停期间遗留的 OPEN Session 保持原状，明确恢复后才允许再次恢复/结算。已有数据仍可查看和删除。已经打开且曾进入暂停状态的搜索页，需要等待页面更新或手动刷新后继续采集。
